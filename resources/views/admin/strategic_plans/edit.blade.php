                ...
                <div class="p-4 border rounded">
                    <form class="row g-3" action="{{ route('strategic_plans.update', $strategicPlan->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea name="description" id="description" rows="10" class="form-control summernote @error('description') is-invalid @enderror" placeholder="Enter Description (optional)">{{ old('description', $strategicPlan->description) }}</textarea>
                            @error('description')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
