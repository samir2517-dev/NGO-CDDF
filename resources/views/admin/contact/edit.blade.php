...
<div class="row">
    <div class="col-xl-10 mx-auto">
        <h6 class="mb-0 text-uppercase">Edit Contact Information</h6>
        <hr/>
        <div class="card">
            <div class="card-body">
                <div class="p-4 border rounded">
                    <form class="row g-3" action="{{ route('contact.update', $contact->id) }}" method="post">
                        @csrf
                        ...
                        <div class="col-md-12" id="title-field">
                            <label for="title" class="form-label">
                                <span id="title-label">Title/Designation</span>
                                <span class="text-danger" id="title-required" style="display: none;">*</span>
                            </label>
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" id="title" placeholder="e.g., Branch Name / Chief Executive" value="{{ $contact->title }}">
                            @error('title')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <!-- Person Fields -->
                        <div id="person-fields" style="display: {{ $contact->type == 'person' ? 'block' : 'none' }};">
                            <div class="col-md-12">
                                <label for="name" class="form-label">Name <span class="text-danger">*</span></label>
                                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="Enter person name" value="{{ $contact->name }}">
                                @error('name')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="mobile" class="form-control @error('mobile') is-invalid @enderror" id="mobile" placeholder="017XXXXXXXX" value="{{ $contact->mobile }}">
                            @error('mobile')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="mobile2" class="form-control" id="mobile2" placeholder="017XXXXXXXX" value="{{ $contact->mobile2 }}">
                        </div>

                        <div class="col-md-6">
                            <label for="email" class="form-label">Email</label>
                            <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" id="email" placeholder="example@email.com" value="{{ $contact->email }}">
                            @error('email')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="email" name="email2" class="form-control" id="email2" placeholder="example@email.com" value="{{ $contact->email2 }}">
                        </div>

                        <!-- Optional Fields for Person only -->
                        <div id="optional-person-fields" style="display: {{ $contact->type == 'person' ? 'flex' : 'none' }};">
                            <div class="col-md-4">
                                <label for="skype" class="form-label">Skype (Optional)</label>
                                <input type="text" name="skype" class="form-control" id="skype" placeholder="Skype ID" value="{{ $contact->skype }}">
                            </div>

                            <div class="col-md-4">
                                <label for="whatsapp" class="form-label">WhatsApp (Optional)</label>
                                <input type="text" name="whatsapp" class="form-control" id="whatsapp" placeholder="WhatsApp number" value="{{ $contact->whatsapp }}">
                            </div>

                            <div class="col-md-4">
                                <label for="twitter" class="form-label">Twitter (Optional)</label>
                                <input type="text" name="twitter" class="form-control" id="twitter" placeholder="@username" value="{{ $contact->twitter }}">
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set initial label based on current type
