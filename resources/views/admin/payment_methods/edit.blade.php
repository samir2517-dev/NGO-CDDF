...
<div class="row">
    <div class="col-xl-10 mx-auto">
        <h6 class="mb-0 text-uppercase">Edit Payment Method</h6>
        <hr/>
        <div class="card">
            <div class="card-body">
                ...
                <div class="p-4 border rounded">
                    <form class="row g-3" action="{{ route('admin.payment_methods.update', $data->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="col-md-8">
                            <div class="col-md-12 mb-3">
                                ...
                                <select name="type" id="type" class="form-select @error('type') is-invalid @enderror" required>
                                    ...
                                    <option value="visa" {{ $data->type == 'visa' ? 'selected' : '' }}>Visa/Card</option>
                                    <option value="bank" {{ $data->type == 'bank' ? 'selected' : '' }}>Bank Account</option>
                                </select>
                                @error('type')
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                @if($data->icon_image)
                                    <div class="mb-2">
                                        <img src="{{ asset('storage/'.$data->icon_image) }}" alt="Current Icon" style="height: 50px;">
                                ...
                                <small class="text-muted">Upload new logo/icon image (Optional, leave empty to keep current)</small>
                                @error('icon_image')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <input type="text" name="account_name" class="form-control @error('account_name') is-invalid @enderror" 
                                       id="account_name" placeholder="e.g., Md. Jane Alam" value="{{ old('account_name', $data->account_name) }}" required>
                                @error('account_name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div id="bank_details" style="display: {{ $data->type == 'bank' ? 'block' : 'none' }};">
                                <div class="col-md-12 mb-3">
                                    <label for="bank_name" class="form-label">Bank Name</label>
                                    <input type="text" name="bank_name" class="form-control" 
                                           id="bank_name" placeholder="e.g., IBBL" value="{{ old('bank_name', $data->bank_details['bank_name'] ?? '') }}">
                                </div>
                                
                                <div class="col-md-12 mb-3">
                                    <label for="branch_name" class="form-label">Branch Name</label>
                                    <input type="text" name="branch_name" class="form-control" 
                                           id="branch_name" placeholder="e.g., Maijdee Court, Noakhali" value="{{ old('branch_name', $data->bank_details['branch_name'] ?? '') }}">
                                </div>
                                
                                <div class="col-md-12 mb-3">
                                    <label for="routing_number" class="form-label">Routing Number</label>
                                    <input type="text" name="routing_number" class="form-control" 
                                           id="routing_number" placeholder="e.g., 125260674" value="{{ old('routing_number', $data->bank_details['routing_number'] ?? '') }}">
                                </div>
                            </div>
